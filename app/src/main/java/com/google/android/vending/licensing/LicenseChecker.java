 ...
 * <p>
 * The LicenseChecker is configured via a {@link Policy} which contains the logic to determine whether a user should have access to the application. For example, the Policy can
 * define a threshold for allowable number of server or client failures before the library reports the user as not having access.
 * <p>
...
public class LicenseChecker implements ServiceConnection {
	 ...
	 * <p>
	 * NOTE: This call uses a trivially obfuscated string (base64-encoded).  For best security, we recommend obfuscating the string that is passed into bindService using another
	...
	public synchronized void checkAccess(LicenseCheckerCallback callback) {
		...
		if (mPolicy.allowAccess()) {
			...
			if (mService == null) {
				...
				try {
					boolean bindResult = mContext.bindService(new Intent(new String(Base64.decode("Y29tLmFuZHJvaWQudmVuZGluZy5saWNlbnNpbmcuSUxpY2Vuc2luZ1NlcnZpY2U="))), this, //
							// ServiceConnection.
	 ...
	 * <p>
	 * Failure to call this method can result in a crash under certain circumstances, such as during screen rotation if an Activity requests the license check or when the user
