...
/**
 * Default policy. All policy decisions are based off of response data received from the licensing service. Specifically, the licensing server sends the following information:
 * response validity period, error retry period, and error retry count.
 * <p>
 * These values will vary based on the the way the application is configured in the Android Market publishing console, such as whether the application is marked as free or is
 * within its refund period, as well as how often an application is checking with the licensing service.
 * <p>
...
public class APKExpansionPolicy implements Policy {
	...
	/**
	 * The design of the protocol supports n files. Currently the market can only deliver two files. To accommodate this, we have these two constants, but the order is the only
	 * relevant thing here.
	 */
	 ...
	 * <p>
	 * This data will be used for computing future policy decisions. The following parameters are processed: <ul> <li>VT: the timestamp that the client should consider the
	 * response
	 * valid until <li>GT: the timestamp that the client should ignore retry errors until <li>GR: the number of retry errors that the client should ignore </ul>
	 *
	...
	/**
	 * {@inheritDoc} This implementation allows access if either:<br> <ol> <li>a LICENSED response was received within the validity period <li>a RETRY response was received in the
	 * last minute, and we are under the RETRY count or in the RETRY period. </ol>
	 */
	...
	/**
	 * Gets the count of expansion URLs. Since expansionURLs are not committed to preferences, this will return zero if there has been no LVL fetch in the current session.
	 *
	...
	/**
	 * Gets the expansion URL. Since these URLs are not committed to preferences, this will always return null if there has not been an LVL fetch in the current session.
	 *
	 * @param index the index of the URL to fetch. This value will be either MAIN_FILE_URL_INDEX or PATCH_FILE_URL_INDEX
	...
	/**
	 * Sets the expansion URL. Expansion URL's are not committed to preferences, but are instead intended to be stored when the license response is processed by the front-end.
	 *
	 * @param index the index of the expansion URL. This value will be either MAIN_FILE_URL_INDEX or PATCH_FILE_URL_INDEX
	 * @param URL   the URL to set
	 */
