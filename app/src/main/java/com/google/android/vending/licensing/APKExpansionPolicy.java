 ...
 * service. Specifically, the licensing server sends the following information:
 * response validity period, error retry period, and error retry count.
 * <p>
 * These values will vary based on the the way the application is configured in the Android Market
 * publishing console, such as whether the application is marked as free or is
 * within its refund period, as well as how often an application is checking with the licensing
 * service.
 * <p>
 * Developers who need more fine grained control over their application's licensing policy should
...
public class APKExpansionPolicy implements Policy {
   ...
   * To accommodate this, we have these two constants, but the order is the only
   * relevant thing here.
   */
   ...
   * <p>
   * This data will be used for computing future policy decisions. The following parameters are
   * processed: <ul> <li>VT: the timestamp that the client should consider the
   * response
   * valid until <li>GT: the timestamp that the client should ignore retry errors until <li>GR: the
   * number of retry errors that the client should ignore </ul>
   *
  ...
  /**
   * {@inheritDoc} This implementation allows access if either:<br> <ol> <li>a LICENSED response was
   * received within the validity period <li>a RETRY response was received in the
