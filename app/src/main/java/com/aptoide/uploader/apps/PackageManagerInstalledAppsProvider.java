  ...
  @Override public Single<InstalledApp> getInstalledApp(String packageName) {
    return Single.fromCallable(
        ...
        .map(info -> {
          ...
          return new InstalledApp(packageInfo.applicationInfo, info.loadLabel(packageManager)
              .toString(), (info.flags & ApplicationInfo.FLAG_SYSTEM) == 1, packageInfo.packageName,
              info.sourceDir, packageInfo.lastUpdateTime, packageInfo.versionCode, false,
              getMainObb(info.packageName), getPatchObb(info.packageName));
        });
  }
