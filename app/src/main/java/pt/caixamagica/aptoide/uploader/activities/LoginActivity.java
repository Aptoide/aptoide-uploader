...
public class LoginActivity extends AppCompatActivity implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, View.OnClickListener,
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == REQUEST_CODE_RESOLVE_ERR && resultCode == RESULT_OK) {
			...
			if (requestCode == 9001 && resultCode == RESULT_OK || requestCode == 90 && resultCode == RESULT_OK) {
				mGoogleApiClient.connect();
			}
		}
	}
	...
	private UserCredentialsJson getStoredUserCredentials() {
		...
		SharedPreferences sharedpreferences = this.getSharedPreferences(SHARED_PREFERENCES_FILE, Context.MODE_PRIVATE);
	...
	public void onConnected(final Bundle bundle) {
		...
		handler.post(new Runnable() {
			...
			public void run() {
				...
				try {
					String token = GoogleAuthUtil.getToken(LoginActivity.this, accountName, "oauth2:server:client_id:" + serverId + ":api_scope:" + Scopes.PLUS_LOGIN);
					userInfo = new UserInfo(accountName, null, token, OAuth2AuthenticationRequest.Mode.google, accountName, null, null, null, 0);
	...
	public void checkStoredCredentialsCallback() {
		...
		if (userCredentialsJson != null) {
			...
			getSupportFragmentManager().beginTransaction().replace(R.id.container, mContent, "loginFragment").commit();
		}
