...
public class SubmitAppFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
		...
		if (savedInstanceState != null) {
			((TextView) rootView.findViewById(R.id.progressBarText)).setText(savedInstanceState.getString("loading"));
	...
	public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
		...
		if (applicationNameEditText.getText().toString().equals("")) applicationNameEditText.setText(selectablePackageInfos.get(0).getLabel());
	}
	...
	public void onSaveInstanceState(Bundle outState) {
		outState.putString("loading", ((TextView) rootView.findViewById(R.id.progressBarText)).getText().toString());
	...
	private void submitApp() throws ValidationException {
		...
		} else Toast.makeText(getActivity(), "Mandatory fields missing", Toast.LENGTH_LONG).show();
	}
	...
	private void retrieveCategorySpinnerArray() {
		...
		if (spinnerArray == null) {
			...
			spiceManager.execute(listCategoriesRequest, new RequestListener<CategoriesJson>() {
				...
				@Override
				public void onRequestSuccess(CategoriesJson categoriesJson) {
					...
					Collections.sort(categories, new Comparator<CategoriesJson.Category>() {
						@Override
						public int compare(CategoriesJson.Category lhs, CategoriesJson.Category rhs) {
	...
	/**
	 * Retorna o indice no spinner da categoria com o <code>id</code> fornecido. 0 caso seja desconhecido.
	 *
	...
	private int getCategory() {
		String selectedItem = (String) ((Spinner) rootView.findViewById(R.id.app_category_spinner)).getSelectedItem();
