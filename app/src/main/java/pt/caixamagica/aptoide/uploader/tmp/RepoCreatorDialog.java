...
public class RepoCreatorDialog extends DialogFragment {
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		builder.setTitle(R.string.repo_creation_title).setView(view).setPositiveButton(R.string.create_repo, new DialogInterface.OnClickListener() {
			@Override
...
//            BusProvider.getInstance().post(new LogoutEvent());
	...
	private boolean validateNotEmptyFields() {
		...
		if (privateButton.isChecked()) {
			if (repoUsername.getText().toString().isEmpty() || repoPassword.getText().toString().isEmpty()) {
				return false;
			}
		}
