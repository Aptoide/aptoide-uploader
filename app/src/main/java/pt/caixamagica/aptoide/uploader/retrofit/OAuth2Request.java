...
public class OAuth2Request {
  ...
  SharedPreferences preferences = context.getSharedPreferences("UploaderPrefs2",
      AptoideUploaderApplication.getContext().MODE_PRIVATE);
  AESObfuscator aesObfuscator = new AESObfuscator(SALT, context.getPackageName(), deviceId);

  public OAuth2Request() {
    try {
      refreshToken =
          aesObfuscator.unobfuscate(preferences.getString("refreshToken", ""), "refreshToken");
  ...
  public String builder() {
    ...
    spiceManager.execute(oAuth2AuthenticationRequest, new RequestListener<OAuth>() {
      ...
      @Override public void onRequestSuccess(OAuth oAuth) {
