...
public class ManelAdapter extends MultiChoiceBaseAdapter {
	...
	ThreadPoolExecutor threadPool = new ThreadPoolExecutor(5, 5, 1, TimeUnit.SECONDS, new PriorityBlockingQueue<Runnable>(5, comparator));
	...
	public ManelAdapter(Bundle savedInstanceState, View view, Fragment fragment, List<SelectablePackageInfo> mDataset, UserCredentialsJson userCredentialsJson) {
	...
	public void setListener(ManelAdapterShowListener listener, View view) {
		...
		if (actionModeActivaded) {
			if (view.findViewById(R.id.submitAppsButton) != null) view.findViewById(R.id.submitAppsButton).setVisibility(View.VISIBLE);
			listener.hide();
		}
	}
	...
	protected View getViewImpl(int position, View convertView, ViewGroup parent) {
		...
		threadPool.execute(new PriorityRunnable() {
			@Override
			public void run() {
				final CharSequence charSequence = apk.applicationInfo.loadLabel(AptoideUploaderApplication.getContext().getPackageManager());
	...
	@Override
	public SelectablePackageInfo getItem(int position) {
